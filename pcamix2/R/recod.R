...
recod <-
  function(X.quanti,X.quali,rename.level=FALSE)
  {
    ...
    if (!is.null(X.quanti)){
      ...
      recodqt <- myrecodquant(as.matrix(X.quanti)) #switch to c func#
      
      Z1 <- recodqt$Z
      #rownames(Z1) = rownames(X.quanti)
      #colnames(Z1) = colnames(X.quanti)
      g1 <- recodqt$g
      #names(g1) = colnames(X.quanti)
      s1 <- recodqt$s
      #names(s1) = colnames(X.quanti)
      Y1 <- recodqt$Xcod
      #rownames(Y1) = rownames(X.quanti)
      #colnames(Y1) = colnames(X.quanti)
    }
    ...
    if (!is.null(X.quali)){
      ...
      g2 <- applym(G,2,"mean") #switch to c func#
      ns <- applym(G,2,"sum") #switch to c func#
      s2 <- sqrt(ns/nrow(G))
      
      Gcod <- sweepD(G,s2,2) #switch to c func#
      moy<-applym(Gcod,2,"mean")
      Z2 <- sweepS(Gcod,moy,2) #switch to c func#
      G.cent<-sweepS(G,ns/nrow(G),2) #switch to c func#
